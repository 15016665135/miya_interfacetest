...
class Logger:
    ...
    level = 'warning'    #设置日志打印级别
    def __init__(self, set_level=level,
                 name=os.path.split(os.path.splitext(sys.argv[0])[0])[-1],
                 log_name=time.strftime("%Y-%m-%d.log", time.localtime()),
                 log_path=os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(os.path.realpath(__file__)))), "log"),
                 use_console=True):
        ...
        '''
            ...
            log_name： 日志文件的名字，默认为当前时间（年-月-日.runlog）
